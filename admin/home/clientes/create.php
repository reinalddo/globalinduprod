...
if (!function_exists('saveOptimizedClientLogo')) {
    function saveOptimizedClientLogo(array $file): string
    {
        ...
        $relativeDir = 'uploads/home/clientes';
        ...
        try {
            $filename = 'client-' . time() . '-' . bin2hex(random_bytes(4)) . '.' . $allowedMimes[$mime];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $optimizedPath = saveOptimizedClientLogo($uploadedLogo);
        } catch (Throwable $exception) {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt->bind_param('ssi', $formData['name'], $optimizedPath, $formData['sort_order']);
            $stmt->execute();
            $stmt->close();
            ...
            if ($optimizedPath !== '') {
                $storedPath = dirname(__DIR__, 3) . '/' . $optimizedPath;
...
<section>
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="form-actions">
            ...
            <a class="btn btn-outline" href="<?php echo adminUrl('home/clientes'); ?>">Cancelar</a>
        </div>
    </form>
</section>
